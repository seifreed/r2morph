...
def _build_mutation_pass(pass_name: str, seed: int) -> MutationPass:
    ...
    if pass_type is AntiDisassemblyPass:
        # ponytail: cap static runtime scans; raise after profiling a bounded alternative.
        config["max_functions"] = _CAMPAIGN_MAX_ANTI_DISASSEMBLY_FUNCTIONS
    if pass_type is CodeVirtualizationPass:
        config["max_function_analysis_count"] = _MATURITY_MAX_FUNCTION_ANALYSIS_COUNT
        return CodeVirtualizationPass(config=config)
    return pass_type(config=config)
