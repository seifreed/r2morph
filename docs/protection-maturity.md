...
transform duration ranged from `0.047` to `1.301` seconds, averaging `0.570`
...
`63.59x`; generated bytecode ranged from `0` to `54,027` bytes. The entrypoint
...
`0.846` mean normalized nearest similarity across seeds. The runtime VM's
...
handler-clustering artifact records the small but reproducible effect.
