...
fixtures cover 17 capability families with no unclassified fixture.
...
| Handler clustering | [`docs/protection-handler-clustering.json`](protection-handler-clustering.json): normalized nearest similarity mean `0.660`, `401` of `2,295` comparisons >= `0.8`, exact normalized matches `0`, with exact-match and above-threshold rates recorded by the report | The remaining similarity is shared handler structure across different operation families, not duplicate normalized bodies; changing the normalizer would be metric gaming |
...
maximum supported boundary. The window now grows from direct region accesses
when a function needs more incoming arguments; non-proven ABI cases still
...
Symbolic resistance measurements also expose an `evidence_status` and
`release_signoff_status` so a cracked original, unavailable backend,
budget-limited virtualized lower bound, and human-review-gated resistance result
cannot be reported as the same strength of evidence.
