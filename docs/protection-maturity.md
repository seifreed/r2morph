...
The current all-fixture adversarial campaign covers 118 executable samples and
the `CodeVirtualization` pass. It records 590 completed tool runs, 472 explicit
unavailable-tool rows, and zero errors. Its per-pass summary records 115 applied
...
The current report records 590 completed tool runs and 472 explicit unavailable-
tool rows after the byte/word memory-width regression. The focused
virtualization inventory is recorded in
[`docs/virtualization-coverage.json`](virtualization-coverage.json): 109 real
...
current fixture inventory contains 118 compatible executable fixtures spanning
...
| Control-flow virtualization | 118-fixture corpus and real exit-code checks | Broad synthetic semantic coverage; production format coverage is narrow |

## 5. Virtualization maturity

The current one-seed adversarial run applied the pass to 115 of 118 fixtures and records
