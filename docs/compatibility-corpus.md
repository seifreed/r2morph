...
AArch64, and x86 32-bit targets currently have four core smoke cases each for
NOP insertion, instruction substitution, register substitution, and a composed
pass sequence; AArch64 additionally exercises constant unfolding of an
architectural zeroing instruction. ARM32 executes under QEMU and AArch64 runs
on native ARM64. This is execution evidence only, not per-pass or full-format
parity.
...
and the aggregate artifact rechecks that all 136 generated variants and all
