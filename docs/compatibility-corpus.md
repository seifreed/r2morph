...
duration pair counts and totals, comparable numeric metric pair counts, and
numeric original-versus-protected metric deltas per analyzer so campaigns
remain comparable by tool.
...
unsupported and partial virtualization capability counts when diagnostics are
present.
...
and totals, static analyzer metric coverage and deltas, and bounded
