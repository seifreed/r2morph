...
Hex-Rays no longer reconstructs the dispatch, handlers, keys, anti-debug probes or
plaintext VM context, but it still recovers the checksum bootstrap and identifies
the appended executable payload.
...
- **Payload container — fragmented.** The injector relocates the program-header
  table into a dedicated read-only load and splits the VM blob across a per-build
  sequence of adjacent RX loads. On `elf_vm_run_callfallback_x86_64`, IDA reports
  one read-only load (`0x402000..0x402150`) and three RX loads of `0x5000`, `0x3000`
  and `0x4d91` bytes instead of one `0xce39` RX container. The adjacent executable
  chain and the interpreter spanning it remain structurally obvious.
...
- **Integrity-dependent prologue — direct-threaded.** After the minimal native
  self-checksum, an encrypted three-entry relative-offset table threads the timing
  probe, tracer probe and key/context setup as independently placed states. Their
  edges are indirect and keyed by the runtime checksum. On the engine recheck below,
  IDA ends the entry function at the first `jmp rax` after 200 bytes and attributes
  none of the syscalls, timestamp reads, tracer scan or frame encryption to it. The
  checksum loop and initial register spill remain visible bootstrap roots.

**Still recovered by the decompiler, and still below the reference protectors:** the
self-checksum loop and initial register spill remain visible before the encrypted
bootstrap transfer; a runtime tracer can follow the later states; and the payload
is structurally obvious (an adjacent appended RX `PT_LOAD` chain).
...
The fragmented-container recheck used the same fixture and seed, producing SHA-256
`26a3e71a15de6e6baa91f2f076775ba24418faadec903525e6cfc1856721b265`.
It preserved exit code 45, moved the program headers into a 336-byte read-only
load, and split 52,625 generated bytes across three adjacent RX loads (`0x5000`,
`0x3000`, `0x4d91`). IDA still recovered the 945-byte linear prologue and stopped
at the encrypted indirect dispatch.

The direct-threaded-bootstrap recheck used the same fixture and seed, producing
SHA-256 `be5a9f35c32abd7172e989020b03abb10cb1e7fd295db0834c761e3fbcf30175`.
It virtualized the same three-instruction run into 52,756 generated bytes and
preserved exit code 45. IDA attributed only 44 instructions (`0x404000..0x4040c8`)
to the entry: register spill, alternating-ends checksum and the checksum-decrypted
indirect jump. No anti-debug or key/context-setup state was recovered as a function.

The matching region/nested recheck used `elf_vm_incall_x86_64`, nesting depth 2,
seed `20260812`, and SHA-256
`99123c36446d0858d355e4de0a1ba398ed638ee0d7d7d1488ace74f54c60c2eb`.
It virtualized two functions (34 instructions, 38,063 generated bytes), preserved
exit code 45, and IDA stopped both entries at their first bootstrap transfer
(`0xc6` and `0xc1` bytes) without attributing either probe sequence.

## What the decompiler recovers

The interpreter entry decompiles through its spill and checksum bootstrap, then
stops at an opaque indirect jump; bootstrap states and handlers are not attributed
to it. Everything below is read directly off the pseudocode, with no manual work:
...
| Tracer probe | not attributed past the encrypted bootstrap transfer |
| Handler count | roughly 40–65 polymorphic instances, not attributed to the dispatcher |

Structural tells at the file level: the payload lands in an adjacent chain of
unnamed RX `PT_LOAD` entries above the image, although IDA no longer resolves the
whole interpreter as one function.
...
variable and can no longer fold the expression. Before bootstrap threading, the
probe decompiled to `filename[0] = 0x0101010101010101 * v_checksum ^ 0x…` rather
than `strcpy(filename, "/proc/self/status")`, and the tracer tag compare read
...
nested interpreter. The self-checksum loop remains visible; the probe now sits
behind the encrypted bootstrap transfer as well.
...
5. **Integrity and anti-debug are observable and centralized** — one checksum byte,
   ...
   **Substantially closed for static recovery.** The anti-debug constants remain
   checksum-keyed, and timing, tracer and key/context setup now execute as three
   direct-threaded states behind an encrypted offset table. IDA stops after the
   checksum-derived indirect jump instead of recovering the centralized probe
   sequence. The checksum range and fold remain visible as the native bootstrap;
   hiding that root would require a different integrity trust anchor, and runtime
   tracing can still recover every state transition.
6. **The payload is structurally obvious**: appended `RX` segments, one contiguous
   ...
   **Partly closed.** The injector now separates the program-header table from the
   executable payload and fragments the payload into per-build adjacent RX loads.
   No single load owns the whole VM blob, but the adjacent chain and the large
   interpreter functions are still immediately visible and remain a maturity gap.
