"""Symbolic-execution report summaries.

Pure, stateless dict/list -> dict/list helpers that aggregate the symbolic
verification metadata recorded on each mutation into the machine-readable
report. Split out of core/report_helpers.py to keep that module within the
file-size budget (CLAUDE.md §14); the parent module re-exports these names so
existing ``r2morph.core.report_helpers`` import paths keep working. Depends
only on core.constants, so it stays usable from both core.engine and the
reporting-layer ReportAssembler without any core -> reporting import (§7).
"""
