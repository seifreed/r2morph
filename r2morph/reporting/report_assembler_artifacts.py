...
from r2morph.core.report_helpers_evidence import _summarize_structural_evidence
...
from r2morph.core.report_helpers_symbolic_summary import _summarize_symbolic_overview
...
class ArtifactBuildRequest:
    """Inputs needed to compute report artifacts."""
...
def build_report_artifacts(request: ArtifactBuildRequest) -> dict[str, Any]:
    """Compute validation, triage, and view artifacts for the report."""
    payload = request.payload
    pass_results = request.pass_results
    ...
    structural_evidence = _summarize_structural_evidence(request.aggregate_structural_regions)
    ...
    pass_support = {mutation.name: mutation.get_support().to_dict() for mutation in request.pipeline_passes}
    pass_capabilities = {
    ...
    normalized_pass_results = _summarize_normalized_pass_results(
        pass_results,
        PassProjectionMaps(
            triage=pass_triage_map,
            capabilities=pass_capability_summary_map,
            validation_roles=validation_role_map,
            evidence=pass_evidence_map,
            symbolic=pass_symbolic_summary,
        ),
    )
    ...
    validation_adjustment_rows = _summarize_validation_adjustment_rows(
        ...
        request.gate_failures if isinstance(request.gate_failures, dict) else {},
    )
    report_views = request.report_view_builder.build_report_views(
        ReportViewInputs(
            ...
            gate_failure_priority=request.gate_failure_priority,
            gate_failure_summary=request.gate_failures if isinstance(request.gate_failures, dict) else {},
            gate_failure_severity_priority=request.gate_failure_severity_priority,
