...
def _render_pass_capabilities(
    *,
    ...
    for pass_name in filtered_summary.get("passes", []):
        ...
        if not capabilities:
            continue
        fragments = build_pass_capability_fragments(capabilities)
        if fragments:
...
def _render_pass_validation_context(
    ...
    fragments = build_pass_validation_context_fragments(context)
    _get_console().print(f"  [cyan]{pass_name}[/cyan]: " + ", ".join(fragments))
...
def _render_only_pass_sections(
    *,
    ...
    if pass_symbolic_summary and pass_symbolic_summary.get("symbolic_requested", 0) > 0:
        ...
        if issues_list:
            issues_by_severity = group_issues_by_severity(issues_list)
    ...
    if pass_region_evidence:
        ...
        for row in pass_region_evidence[:5]:
            region = build_pass_region_label(row.get("start_address"), row.get("end_address"))
            _get_console().print(
                "  "
                f"[cyan]{region}[/cyan]: "
