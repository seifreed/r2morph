...
_FP_VEX_256_VARIABLE_BLEND: dict[str, str] = {
    "vblendvps": "blendvps",
    "vblendvpd": "blendvpd",
}
...
def _decode_fp_vex_256_variable_blend(text: str) -> tuple[str, str, int, int, int, int] | None:
    """Decode a VEX.256 blend selected by a vector mask register."""
    parts = text.split(None, 1)
    if len(parts) != _INSTRUCTION_PART_COUNT:
        return None
    operation = _FP_VEX_256_VARIABLE_BLEND.get(parts[0].lower())
    if operation is None:
        return None
    operands = [token.strip() for token in parts[1].split(",")]
    if len(operands) != _VEX_256_PERMUTE_OPERAND_COUNT:
    ...
    destination, first_source, second_source, mask = registers
...
def _decode_fp_vex_packed_mem(text: str, insn_addr: int, insn_size: int) -> tuple[Any, ...] | None:
    """Decode a VEX.128 packed move between an XMM register and memory."""
    parts = text.split(None, 1)
    if len(parts) != _INSTRUCTION_PART_COUNT or parts[0].lower() not in _FP_VEX_PACKED_MOVE:
        return None
    left, right = (token.strip() for token in parts[1].split(",", 1))
    ...
    if left_mem == right_mem:
        return None
    if left_mem:
        kind, mem_text, xmm_text = "fpstorevexpacked", left, right
    else:
        kind, mem_text, xmm_text = "fploadvexpacked", right, left
    xmm_index = _parse_xmm_operand(xmm_text)
    if xmm_index is None:
        return None
    normalized_mem = mem_text.lower().replace("xmmword", "")
    result: tuple[Any, ...] | None = None
    indexed = _parse_indexed_operand(normalized_mem, base_optional=True)
    if indexed is not None:
        base_slot, index_slot, shift, displacement = indexed
        if base_slot < 0:
            result = (kind + "idxnb", xmm_index, index_slot, shift, displacement)
        else:
            result = (kind + "idx", xmm_index, base_slot, index_slot, shift, displacement)
    else:
        parsed_rip = _parse_riprel_operand(normalized_mem, insn_addr, insn_size)
        if parsed_rip is not None:
            result = (kind + "rip", xmm_index, parsed_rip[0])
        else:
            memory = _parse_mem_operand(normalized_mem)
            if memory is not None:
                base_slot, displacement, _width = memory
                result = (kind, xmm_index, base_slot, displacement)
    return result
