"""I/O helpers for benchmark reporting exports."""
...
def build_json_export_payload(results: list[BenchmarkResult]) -> dict[str, Any]:
    """Build the JSON payload for benchmark exports."""
    return {
        "metadata": {
...
def write_json_export(results: list[BenchmarkResult], output_path: str) -> None:
    """Write benchmark results to JSON."""
    with open(output_path, "w") as f:
        json.dump(build_json_export_payload(results), f, indent=2, default=str)


def write_csv_export(results: list[BenchmarkResult], output_path: str) -> None:
