...
def _fde(eh_frame_vaddr: int, blob_vaddr: int, blob_size: int, frame_size: int) -> bytes:
    cie = _cie()
    fde_vaddr = eh_frame_vaddr + len(cie)
    content_start = fde_vaddr + 4
    cie_pointer = content_start - eh_frame_vaddr
    instructions = bytes((_DW_CFA_ADVANCE_LOC1, _SUB_RSP_IMMEDIATE_BYTES, _DW_CFA_DEF_CFA_OFFSET)) + _uleb128(
        frame_size + 8
    )
    body = (
...
def build_vm_eh_frame(blob_vaddr: int, blob_size: int, frame_size: int, metadata_vaddr: int) -> bytes:
    ...
    ``sub rsp, immediate`` instruction. LSDA-bearing call sites are intentionally
    outside this contract; callers must reject those shapes before using it.
    """
    ...
    eh_frame = _fde(eh_frame_vaddr, blob_vaddr, blob_size, frame_size)
