...
_EXPECTED_0X88_256 = 0x100
_EXPECTED_OFF_136 = 0x88
_EXPECTED_S_FLAGS_OFFSET_128 = 0x80
_EXPECTED_S_FLAGS_OFFSET_128_2 = 0x80
...
def test_scheme_gp_slots_leave_a_hole_in_the_contiguous_context_array() -> None:
    schemes = [build_region_scheme(_tiny_region(), randomness.Random(seed)) for seed in range(64)]
    expect(all(set(range(16)) - set(scheme.slot_perm) for scheme in schemes))


def test_scheme_gp_slots_use_safe_outlier_cells() -> None:
    schemes = [build_region_scheme(_tiny_region(), randomness.Random(seed)) for seed in range(64)]
    expect(all(set(scheme.slot_perm) & {0x90 // 8, 0xA8 // 8} for scheme in schemes))
...
def test_handlers_position_unmask_their_immediates() -> None:
    ...
    expect(re.search(rf"imul (\w+), \1, {re.escape(hex(_DWORD_BROADCAST))}", asm))
...
def test_checksum_slot_is_relocated_per_build_within_the_free_frame_gap() -> None:
    ...
    offsets = {build_region_scheme(_tiny_region(), randomness.Random(seed)).checksum_offset for seed in range(64)}
    expect(not (len(offsets) <= 1))
    expect(all(_EXPECTED_OFF_136 <= off < _EXPECTED_0X88_256 and off % 8 == 0 for off in offsets))
...
def test_interpreter_folds_the_relocated_checksum_slot() -> None:
    ...
    scheme = build_region_scheme(region, randomness.Random(0))
    asm = _interpreter_asm(region, scheme)
    expect(not (f"[rsp+{scheme.checksum_offset}]" not in asm))
...
def test_flags_slot_is_relocated_per_build_distinct_from_checksum() -> None:
    ...
    schemes = [build_region_scheme(_tiny_region(), randomness.Random(seed)) for seed in range(64)]
    expect(not (len({s.flags_offset for s in schemes}) <= 1))
    expect(
        all(
            _EXPECTED_S_FLAGS_OFFSET_128 <= s.flags_offset < _EXPECTED_0X80_256 and s.flags_offset % 8 == 0
            for s in schemes
        )
    )
    expect(all(s.flags_offset != s.checksum_offset for s in schemes))
...
def test_interpreter_relocates_the_flags_slot_off_the_canonical_offset() -> None:
    ...
    scheme = next(
        s
        for s in (build_region_scheme(region, randomness.Random(seed)) for seed in range(64))
        if s.flags_offset != _EXPECTED_S_FLAGS_OFFSET_128_2
    )
    asm = _interpreter_asm(region, scheme)
    expect(not (re.search(r"\[rsp\s*\+\s*128\]", asm)))
    expect(not (f"[rsp + {scheme.flags_offset}]" not in asm))
