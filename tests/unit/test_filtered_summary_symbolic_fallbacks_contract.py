...
def test_build_filtered_summary_symbolic_fallback_sections_derives_rows_from_by_pass() -> None:
    ...
    expect(
        fallback_sections["symbolic_issue_passes"]
        == [
            {
    ...
    expect(
        fallback_sections["symbolic_coverage_by_pass"]
        == [
            {
    ...
    expect(
        fallback_sections["symbolic_severity_by_pass"]
        == [
            {"pass_name": "coverage-pass", "severity": "bounded-only", "issue_count": 0, "symbolic_requested": 2},
            {"pass_name": "mismatch-pass", "severity": "mismatch", "issue_count": 2, "symbolic_requested": 3},
        ]
    )
...
def test_build_filtered_summary_symbolic_fallback_sections_handles_empty_input() -> None:
    expect(
        _build_filtered_summary_symbolic_fallback_sections(by_pass={})
        == {"symbolic_issue_passes": [], "symbolic_coverage_by_pass": [], "symbolic_severity_by_pass": []}
    )
