...
def test_cli_path_resolution_prefers_explicit_options() -> None:
    ...
    expect(resolved_input == input_path)
    expect(resolved_output == output_path)
...
def test_cli_path_resolution_uses_positionals_when_options_missing() -> None:
    ...
    expect(resolved_input == Path("positional-in.bin"))
    expect(resolved_output == Path("positional-out.bin"))
