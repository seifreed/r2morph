...
def test_missing_previous_binary_path_returns_no_checkpoint() -> None:
    result = SymbolicValidator()._compare_real_binary_regions(SimpleNamespace(path="/whatever"), {}, SimpleNamespace())
    assert result == {
...
def test_missing_current_binary_path_returns_path_not_available(tmp_path: Path) -> None:
    ...
    result = SymbolicValidator()._compare_real_binary_regions(
