...
def test_checksum_step_spans_every_op_rotate_and_amount() -> None:
    ...
    assert {op for op, _, _ in steps} == set(_CHECKSUM_OPS)
    assert {rot for _, rot, _ in steps} == set(_CHECKSUM_ROTATES)
    assert {amount for _, _, amount in steps} == set(range(1, 8))
...
def test_compute_build_checksum_is_byte_sensitive() -> None:
    ...
    flipped[30] ^= 0x01
    assert compute_build_checksum(base, 7) != compute_build_checksum(bytes(flipped), 7)
...
def test_compute_build_checksum_varies_by_variant() -> None:
    ...
    assert compute_build_checksum(code, 0) != compute_build_checksum(code, 20)
