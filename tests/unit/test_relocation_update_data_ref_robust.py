...
def test_degenerate_pointer_read_returns_false_without_crash(p8_response: str | None) -> None:
    ...
    expect(not (rm._update_data_ref(0x1000, 0xDEAD, 0xBEEF) is not False))
...
def test_matching_pointer_is_rewritten() -> None:
    ...
    expect(not (rm._update_data_ref(0x2000, 0x11223344, 0x55667788) is not True))
    expect(binary.writes == [(8192, (1432778632).to_bytes(4, "little"))])
...
def test_non_matching_pointer_is_left_untouched() -> None:
    ...
    expect(not (rm._update_data_ref(0x2000, 0x99999999, 0x55667788) is not False))
