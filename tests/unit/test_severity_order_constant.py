...
def test_severity_order_canonical_mapping() -> None:
    expect(
        core_constants.SEVERITY_ORDER
        == {"mismatch": 0, "without-coverage": 1, "bounded-only": 2, "clean": 3, "not-requested": 4}
    )


def test_severity_order_is_a_single_shared_object() -> None:
    expect(not (gate_evaluator.SEVERITY_ORDER is not core_constants.SEVERITY_ORDER))
