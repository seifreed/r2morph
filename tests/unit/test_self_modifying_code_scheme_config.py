...
def test_string_scheme_is_coerced_to_enum() -> None:
    expect(not (SelfModifyingCodePass({"encryption_scheme": "rc4"}).encryption_scheme is not EncryptionScheme.RC4))
    expect(
        not (
            SelfModifyingCodePass({"encryption_scheme": "xor_rolling"}).encryption_scheme
            is not EncryptionScheme.XOR_ROLLING
        )
    )
...
def test_enum_scheme_passes_through() -> None:
    ...
    expect(not (p.encryption_scheme is not EncryptionScheme.ADD_SUB))


def test_invalid_scheme_falls_back_to_default() -> None:
    p = SelfModifyingCodePass({"encryption_scheme": "totally-not-a-scheme"})
    expect(not (p.encryption_scheme is not EncryptionScheme.XOR_KEY))


def test_default_scheme_is_enum() -> None:
    expect(not (SelfModifyingCodePass().encryption_scheme is not EncryptionScheme.XOR_KEY))
...
def test_string_scheme_runs_in_pipeline(tmp_path: Path) -> None:
    ...
    expect(result["failed_passes"] == 0, f"SMC failed: {result.get('pass_results')}")
    for entry in result.get("pass_results", []):
        if isinstance(entry, dict):
            err = str(entry.get("error") or "")
            expect("has no attribute 'value'" not in err, f"scheme-config regressed: {err}")
