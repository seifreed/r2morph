...
def test_release_closes_lock_file_even_if_not_locked(tmp_path: Path) -> None:
    ...
    expect(lock_file is not None and not lock_file.closed)
    ...
    expect(lock_file.closed, "release() leaked the lock file when not _locked")
...
def test_abandoned_lock_does_not_leak_fd(tmp_path: Path) -> None:
    ...
    expect(lock_file.closed, "abandoned BinaryFileLock leaked its fd (no __del__)")
