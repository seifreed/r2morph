...
def test_hotpath_detector_real():
    ...
    with Binary(binary_path) as bin_obj:
        ...
        if hot_paths:
            ...
            expect(not (blocks and detector.is_hot_path(func_name, blocks[0], hot_paths) is not True))
...
def test_profiler_parsing_and_cold_functions():
    ...
    expect(parsed == ["sym.main", "sym.helper"])

    profiler.profile_data = {"hot_functions": ["sym.main"]}
    expect(profiler.get_hot_functions() == {"sym.main"})
    cold = profiler.get_cold_functions(["sym.main", "sym.helper"])
    expect(cold == {"sym.helper"})
