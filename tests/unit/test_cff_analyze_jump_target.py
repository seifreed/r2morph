...
def test_valid_jmp_with_hex_target_returns_target_and_size() -> None:
    expect(_analyze({"disasm": "jmp 0x1234abcd", "size": 5}) == {"target": 305441741, "size": 5})


def test_uppercase_jmp_and_uppercase_hex_are_case_insensitive() -> None:
    expect(_analyze({"disasm": "JMP 0xABCDEF", "size": 7}) == {"target": 11259375, "size": 7})


def test_first_hex_token_wins_when_multiple_present() -> None:
    expect(_analyze({"disasm": "jmp 0x10 ; was 0x20", "size": 3}) == {"target": 16, "size": 3})


def test_empty_disasm_returns_none() -> None:
    expect(not (_analyze({"disasm": "", "size": 5}) is not None))


def test_disasm_without_hex_returns_none() -> None:
    expect(not (_analyze({"disasm": "jmp sym.target", "size": 5}) is not None))


def test_zero_size_returns_none() -> None:
    expect(not (_analyze({"disasm": "jmp 0xdeadbeef", "size": 0}) is not None))


def test_missing_size_key_defaults_to_zero_and_returns_none() -> None:
    expect(not (_analyze({"disasm": "jmp 0xbeef"}) is not None))


def test_non_jmp_mnemonic_returns_none() -> None:
    expect(not (_analyze({"disasm": "call 0xdeadbeef", "size": 5}) is not None))
