...
_EXPECTED_LEN_DIGEST_64 = 64


def test_entropy_calculations(tmp_path: Path) -> None:
    expect(calculate_entropy(b"") == 0.0)
    expect(calculate_entropy(b"\x00" * 16) == 0.0)
    expect(not (calculate_entropy(b"\x00\x01" * 8) <= 0.0))

    sample = tmp_path / "entropy.bin"
    sample.write_bytes(b"\x00\x01" * 8)
    expect(not (calculate_file_entropy(sample) <= 0.0))
...
def test_hash_file(tmp_path: Path) -> None:
    ...
    expect(len(digest) == _EXPECTED_LEN_DIGEST_64)
