...
from r2morph.mutations.code_virtualization_region import _classify
...
def test_classify_pushfq_yields_fsave_when_opted_in() -> None:
    ...
    assert _classify({"type": "upush", "opcode": "pushfq"}, allow_computed_jump=True) == ["fsave"]
...
def test_classify_popfq_yields_frestore_when_opted_in() -> None:
    ...
    assert _classify({"type": "upop", "opcode": "popfq"}, allow_computed_jump=True) == ["frestore"]
...
def test_frestore_op_key_is_its_own_handler_family() -> None:
    ...
    assert _op_key(("frestore",)) == "frestore"
...
def test_flag_transfer_items_encode_as_a_single_opcode_byte() -> None:
    ...
    assert _item_size(("fsave",)) == 1
    assert _item_size(("frestore",)) == 1
