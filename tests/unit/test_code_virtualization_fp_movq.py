...
def test_fp_memory_decoder_movd_load_uses_dword_width() -> None:
    expect(_decode_fp_mem("movd xmm1, dword ptr [rax+8]") == ("fpload", 1, 0, 8, 32))


def test_vector_classifier_virtualizes_memory_movd_load() -> None:
    expect(
        _classify(
            {
                "type": "mov",
                "family": "vec",
                "opcode": "movd xmm1, dword ptr [rax+8]",
                "addr": 0x1000,
                "size": 8,
            }
        )
...
def test_fp_rip_relative_decoder_movd_load_uses_dword_width() -> None:
    expect(_decode_fp_riprel("movd xmm1, dword ptr [rip+8]", 0x1000, 8) == ("fploadrip", 1, 0x1010, 32))
...
def test_fp_indexed_decoder_movd_load_uses_dword_width() -> None:
    expect(_decode_fp_indexed("movd xmm1, dword ptr [rax + rdx*4 + 16]") == ("fploadidx", 1, 0, 2, 2, 16, 32))
