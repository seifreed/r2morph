...
def test_block_reordering_helpers():
    ...
    expect(reorder._can_reorder_function(func, blocks))
...
def test_control_flow_flattening_helpers():
    ...
    expect(cff._is_conditional_jump("je", "x86"))
    expect(cff._is_conditional_jump("b.ne", "arm"))
    expect(not (cff._is_conditional_jump("jmp", "x86")))
...
def test_instruction_expansion_helpers():
    ...
    expect(built is None or "xor" in built)
...
def test_register_substitution_helpers():
    ...
    expect(isinstance(lea_safe, bool))
