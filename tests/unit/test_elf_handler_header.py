...
def test_parse_elf_header_exact_real_elf64() -> None:
    ...
    expect(not (handler._is_64bit is not True))
    expect(not (handler._is_little_endian is not True))


def test_parse_elf_header_is_cached() -> None:
    handler = ELFHandler(Path("fixtures/dataset/elf_x86_64"))
    expect(not (handler._parse_elf_header() is not handler._parse_elf_header()))
