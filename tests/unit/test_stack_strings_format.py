...
def test_nul_byte():
    expect(_format_plain_stack_byte(5, 0, "rsp") == "    mov byte [rsp+5], 0")


def test_printable_char():
    expect(_format_plain_stack_byte(0, ord("A"), "rsp") == "    mov byte [rsp+0], 'A'")


def test_single_quote_is_escaped_numerically():
    expect(_format_plain_stack_byte(1, ord("'"), "esp") == "    mov byte [esp+1], 39  ; '''")


def test_backslash_is_escaped_numerically():
    expect(_format_plain_stack_byte(2, ord("\\"), "rsp") == "    mov byte [rsp+2], 92  ; '\\'")


def test_non_printable_byte_as_hex():
    expect(_format_plain_stack_byte(3, 255, "esp") == "    mov byte [esp+3], 0xFF")


def test_boundary_space_is_hex():
    # 32 (space) is not in (32, 128), so it falls through to hex
    expect(_format_plain_stack_byte(4, 32, "rsp") == "    mov byte [rsp+4], 0x20")
