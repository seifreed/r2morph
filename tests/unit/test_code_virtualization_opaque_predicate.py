...
from r2morph.mutations.code_virtualization_region_codegen import _opaque_predicate_asm
...
def test_opaque_predicate_has_the_always_even_guard_shape() -> None:
    asm = _opaque_predicate_asm(random.Random(1), 7)
...
def test_opaque_predicate_label_tracks_the_index() -> None:
    # The skip label must be keyed on the handler index so it stays unique across
    # handler instances and nested layers.
    assert "jz opaque_42\n" in _opaque_predicate_asm(random.Random(1), 42)


@pytest.mark.parametrize("seed_value", _SEEDS)
def test_opaque_predicate_branch_is_always_taken(seed_value: int) -> None:
    ...
    raw = _opaque_predicate_asm(random.Random(seed_value & 0xFF), 0)
