...
def _edge_operands(width: int) -> tuple[int, ...]:
    ...
    rng = randomness.Random(0xC0FFEE01)
...
def _regs() -> tuple[int, int, int, int]:
    ...
    return u_c__x86__r_e_g__r_a_x, u_c__x86__r_e_g__r_b_x, u_c__x86__r_e_g__r_b_p, u_c__x86__r_e_g__r11
...
def _cpu_flags(op: str, width: int, a: int, b: int) -> int:
    ...
    out = _run(asm, {u_c__x86__r_e_g__r_a_x: a, u_c__x86__r_e_g__r_c_x: b}, [u_c__x86__r_e_g__r_d_x])
    return out[u_c__x86__r_e_g__r_d_x]


def _compare_flags(*values: int | str) -> int:
    """RFLAGS from the full per-build compare computation + flag synthesis."""
    op, width, arith_variant, compare_variant, a, b = values
...
def test_compare_variant_zero_keeps_the_inline_canonical_handler() -> None:
    ...
    expect("lea r10, [r10 + 1]" not in canonical)
    expect(nonzero != canonical)


def test_isa_spec_zero_seed_is_canonical_and_seeds_diverge() -> None:
    expect(build_isa_spec(0).compare_variant == 0)
